...
date: "2024-04-04"
categories: [Portfolio, DataViz, Network, iGraph, assignment, reference]
image: "julia3-7.png"
description: "iGRAPH!"
code-fold: true
code-tools: true
...
plot(g,  layout = layout_with_fr, main = "The Original Graph")
...
#colors
department_colors <- c("green", "violet", "orange", "turquoise", "gold", "salmon")

V(g)$color <- department_colors[match(V(g)$department, unique(V(g)$department))]

#Labels
#Order: Barrie, Konrad, Ed, Robyn, Scott, Heidi, Lucas, Yaotian, Ymin, Jian, Geraline
offsets <- matrix(c(-2.1, -3.3, 3, 2.2, -2.7, 3.7, 3.9, 4.3, 3.2, -2.5, 2.5), ncol = 2, byrow = TRUE)
angles <- matrix(c(-pi/2.2, -pi/5, 0, pi/2, pi/2, 0, pi/7, 0, pi/6, -pi/2, -pi/2), ncol = 2, byrow = TRUE, )

V(g)$label.color <- "darkblue"  # Set vertex label color to black
V(g)$label.font <- 4         # Set vertex label font to bold


plot(g,  layout = layout_with_kk, rescale = TRUE,
     vertex.label.dist = offsets, 
     vertex.label.degree = angles,
     vertex.size = 15,
     edge.color = ifelse(E(g)$Advisor == TRUE, "brown", "pink"),
     edge.width = ifelse(E(g)$Advisor == TRUE, 4, 2),
     main = "The Improved Graph")

# Create legend using the same colors
legend("bottomleft", legend = unique(people$department), fill = department_colors, title = "Department")
...
3.  Adjusting the labels to improve readability.
